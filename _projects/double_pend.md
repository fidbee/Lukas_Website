...
Overall the model had pretty good correlation with the experimental data. Error bars were generated for the theoretical data based on a Monte Carlo simulation, where the uncertainty of each parameter (mass, lengths, starting angle, etc.) was taken into account. Experimental uncertainty was measured based on the standard deviation over multiple trials. The location of the second linkage especially deviates from the theoretical model over time. This is probably because of the frictionless bearing assumption in the model.